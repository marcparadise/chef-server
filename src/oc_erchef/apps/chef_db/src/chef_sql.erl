...
ping() ->
    try
        case sqeache_client:select(erchef,erchef,ping, [], rows_as_scalars, [ping]) of
            {ok, [<<"pong">>]} -> pong;
            _Else -> throw(pang)
        end
    catch
...
%% Return a node count
count_nodes() ->
    sqeache_client:select(erchef,erchef,count_nodes, [], first_as_scalar, [count]).
...
fetch_org_metadata(OrgName) ->
    ...
    case sqeache_client:select(erchef,erchef,find_organization_by_name, [OrgName], first, []) of
        {ok, none} ->
            not_found;
        {ok, L} when is_list(L) ->
...
is_user_in_org(UserName, OrgName) ->
    case sqeache_client:select(erchef,erchef,user_in_org, [UserName, OrgName], first_as_scalar, [count]) of
        {ok, 0} ->
            false;
...
list_common_orgs_for_users(User1Id, User2Id) ->
    case sqeache_client:select(erchef,erchef,list_common_orgs_for_users, [User1Id, User2Id], rows) of
        {ok, none} ->
            [];
        {ok, L} when is_list(L) ->
            L;
...
fetch_actors_by_name(OrgId, Name) ->
    ...
    Result = sqeache_client:select(erchef,erchef,fetch_requestors_by_name, [OrgId, Name], Transform),
    match_result(Result).
...
fetch_data_bag_item_ids(OrgId, DataBagName) ->
    ...
    case sqeache_client:select(erchef,erchef,QueryName, [OrgId, DataBagName], rows_as_scalars, [id]) of
        {ok, L} when is_list(L) ->
            {ok, L};
        {ok, none} ->
            {ok, []};
...
bulk_get_clients(ApiVersion, Ids) ->
    ...
    case sqeache_client:select(erchef,erchef,Query, [Ids], ?ALL(chef_client)) of
        {ok, none} ->
            {ok, not_found};
        {ok, L} when is_list(L) ->
...
fetch_latest_cookbook_versions(OrgId, CookbookName, NumberOfVersions) ->
    Result = case {CookbookName, NumberOfVersions} of
                 {all, all} ->
                     ...
                     sqeache_client:select(erchef,erchef,fetch_all_cookbook_versions_by_orgid, [OrgId]);
                 {all, _} ->
                     %% Some versions of all cookbooks
                     sqeache_client:select(erchef,erchef,fetch_recent_cookbook_versions_by_orgid_and_num, [OrgId, NumberOfVersions]);
                 {_, all} ->
                     %% All versions of one cookbook
                     sqeache_client:select(erchef,erchef,fetch_all_cookbook_versions_by_orgid_cookbook, [OrgId, CookbookName])
             end,
    case Result of
        {ok, none} ->
            {ok, []};
...
fetch_latest_cookbook_recipes(OrgId) ->
    case sqeache_client:select(erchef,erchef,fetch_latest_cookbook_recipes_by_orgid, [OrgId]) of
        {ok, none} ->
            {ok, []};
...
fetch_all_cookbook_version_dependencies(OrgId) ->
    case sqeache_client:select(erchef,erchef,fetch_all_cookbook_version_dependencies_by_orgid, [OrgId]) of
        {ok, none} ->
            {ok, []};
        {ok, Rows} ->
...
bulk_fetch_minimal_cookbook_versions(OrgId, CookbookVersions) ->
    ...
    case sqeache_client:select(erchef,erchef,bulk_fetch_minimal_cookbook_versions, [OrgId, QueryParam], ?ALL(chef_cookbook_version)) of
        {ok, none} ->
            [];
        {ok, Results} ->
            Results;
...
fetch_cookbook_version(OrgId, {Name, {Major, Minor, Patch}}) ->
    ...
    case sqeache_client:select(erchef,erchef,QueryName, [OrgId, Name, Major, Minor, Patch], FirstRecordTxfm) of
...
fetch_latest_cookbook_version(OrgId, CookbookName) ->
  ...
  case sqeache_client:select(erchef,erchef,QueryName, [OrgId, CookbookName], FirstRecordTxfm) of
    {ok, none} ->
      not_found;
...
select_rows({Query, BindParameters}) ->
    match_result(sqeache_client:select(erchef,erchef,Query, BindParameters));
select_rows({Query, BindParameters, Transform}) when is_tuple(Transform);
                                                     Transform == rows ->
    match_result(sqeache_client:select(erchef,erchef,Query, BindParameters, Transform));
select_rows({Query, BindParameters, Fields = [_|_]}) ->
    match_result(sqeache_client:select(erchef,erchef,Query, BindParameters, rows_as_scalars, Fields)).
...
mark_checksums_as_uploaded(OrgId, [Checksum|Rest]) ->
    case sqeache_client:statement(erchef,insert_checksum, [OrgId, Checksum], count) of
        {ok, 1} ->
...
non_uploaded_checksums(SandboxId, OrgId) when is_binary(SandboxId),
                                              is_binary(OrgId) ->
    case sqeache_client:select(erchef,erchef,non_uploaded_checksums, [OrgId, SandboxId], rows_as_scalars, [checksum]) of
        {ok, none} ->
            [];
...
fetch_object(Keys, RecordName, QueryName, RecordFields) ->
    ...
    case sqeache_client:select(erchef,erchef,QueryName, Keys, FirstRecordTxfm) of
...
bulk_get_objects(Type, Ids) ->
    ...
    case sqeache_client:select(erchef,erchef,Query, [Ids], rows_as_scalars, [serialized_object]) of
        {ok, none} ->
            {ok, not_found};
        {ok, L} when is_list(L) ->
            {ok, L};
...
bulk_get_authz_ids(Type, Ids) ->
    ...
    case sqeache_client:select(erchef,erchef,Query, [Ids], rows, []) of
        {ok, none} ->
            {ok, not_found};
        {ok, L} when is_list(L) ->
            {ok, L};
...
create_object(insert_cookbook_artifact_version = QueryName, Args) when is_list(Args) ->
    case sqeache_client:select(erchef,erchef,QueryName, Args) of
        {ok, [_Row]} -> {ok, 1};
...
create_object(QueryName, Args) when is_atom(QueryName), is_list(Args) ->
    sqeache_client:statement(erchef,QueryName, Args, count);
...
insert_cookbook_checksums([Checksum|Rest], OrgId, Name, Major, Minor, Patch) ->
    case sqeache_client:statement(erchef,insert_cookbook_version_checksum, [Checksum, OrgId, Name, OrgId, Major, Minor, Patch], count) of
        {ok, 1} ->
...
unlink_checksums_from_cbv([Checksum|Rest], OrgId, CookbookVersionId) ->
    case sqeache_client:statement(erchef,delete_cookbook_version_checksum, [Checksum, OrgId, CookbookVersionId], count) of
        {ok, _Count} ->
...
-spec delete_object(delete_query(), binary()) -> {ok, 1 | 'not_found'} | any().
delete_object(delete_sandbox_by_id = Query, Id) ->
    ...
    case sqeache_client:statement(erchef,Query, [Id]) of
        {ok, N} when is_integer(N) ->
...
delete_object(delete_cookbook_by_orgid_name = Query, OrgId, Name) ->
    ...
    case sqeache_client:statement(erchef,Query, [OrgId, Name]) of
        {ok, 1} ->
            {ok, 1};
        {ok, none} ->
            {ok, not_found};
...
do_update(QueryName, UpdateFields) ->
    case sqeache_client:statement(erchef,QueryName, UpdateFields) of
        {ok, 1} -> {ok, 1};
...
insert_sandboxed_checksums([Checksum|Rest], OrgId, SandboxId, CreatedAt) ->
    case sqeache_client:statement(erchef,insert_sandboxed_checksum, [OrgId, SandboxId, Checksum, CreatedAt], count) of
        {ok, 1} ->
...
create_cookbook_if_needed(false, #chef_cookbook_version{authz_id = AuthzId,
                                                        org_id = OrgId,
                                                        name = Name}) ->
    case sqeache_client:statement(erchef,insert_cookbook, [AuthzId, OrgId, Name], count) of
        {ok, N} when is_integer(N) ->
            ok;
        Error ->
            Error
...
fetch_cookbook_version_checksums(OrgId, CookbookVersionId) when is_binary(OrgId),
    ...
    case sqeache_client:select(erchef,erchef,QueryName, [OrgId, CookbookVersionId], rows_as_scalars, [checksum]) of
        {ok, none} ->
            [];
        {ok, Checksums} when is_list(Checksums) ->
            Checksums;
...
fetch_cookbook_authz(OrgId, CookbookName) ->
    case sqeache_client:select(erchef,erchef,find_cookbook_by_orgid_name,
...
unlink_all_checksums_from_cbv(OrgId, CookbookVersionId) ->
    ...
    case sqeache_client:statement(erchef,delete_cookbook_checksums_by_orgid_cookbook_versionid,
...
delete_orphaned_checksums(OrgId, Checksums) ->
    ...
    lists:foldl(fun(Checksum, Acc) ->
            case sqeache_client:statement(erchef,delete_checksum_by_id, [OrgId, Checksum]) of
...
cookbook_versions_from_db(QueryName, Args) ->
    case sqeache_client:select(erchef,erchef,QueryName, Args, rows, []) of
        {ok, L} when is_list(L) ->
            {ok,
...
create_cookbook_version_dict(OrgId) ->
    case sqeache_client:select(erchef,erchef,fetch_all_cookbook_version_ids_by_orgid, [OrgId]) of
        {ok, none} ->
            {ok, dict:new()};
...
fetch_cookbook_version_serialized_objects_batch(Ids) when is_list(Ids)->
    case sqeache_client:select(erchef,erchef,bulk_get_cbv_serialized_object, [Ids]) of
        {ok, none} ->
            {ok, []};
        {ok, Results} ->
...
list_all_policy_revisions_by_orgid(OrgId) ->
    case sqeache_client:select(erchef,erchef,list_all_policy_revisions_by_orgid, [OrgId]) of
        {ok, none} -> {ok, []};
        {ok, AllRevisionRows} ->
...
find_all_policy_revisions_by_group_and_name(OrgId) ->
    case sqeache_client:select(erchef,erchef,find_all_policy_revisions_by_group_and_name, [OrgId]) of
        {ok, none} ->
            {ok, []};
        {ok, Rows } ->
...
find_all_policy_revisions_associated_to_group(OrgId, GroupName) ->
    case sqeache_client:select(erchef,erchef,find_all_policy_revisions_associated_to_group, [OrgId, GroupName]) of
        {ok, none} ->
            {ok, []};
        {ok, Rows } ->
            Processed = policy_rev_by_group_rows_to_tuple(Rows, []),
...
proplist_results(Query, Args) ->
    case sqeache_client:select(erchef,erchef,Query, Args) of
        {ok, L} when is_list(L) ->
            L;
        {ok, none} ->
            [];
